...
export default function ComercialesPanelScreen({
 ...
 <div className={`h-[100dvh] min-h-0 bg-[#030308] text-slate-100 flex flex-col font-sans relative overflow-hidden ${commercialTheme === 'light' ? 'commercial-theme-light' : 'commercial-theme-dark'}`}>
  ...
  <div className={`${activeView === 'pipeline' ? 'block' : 'hidden lg:block'} bg-gradient-to-r from-amber-500/10 via-violet-500/10 to-cyan-500/10 border border-amber-500/15 rounded-2xl p-4 sm:p-5 overflow-hidden relative`}>
   <div className="legacy-rank-visual absolute right-3 top-2 flex h-20 w-20 items-center justify-center sm:right-5 sm:top-3 sm:h-24 sm:w-24" style={{ '--legacy-rank-mask': `url("${myLegacy.rank.asset}")` } as React.CSSProperties}>
