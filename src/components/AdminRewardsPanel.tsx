...
import { buildSalesRewards, calculateLegacyPoints } from '../utils/salesRewards';
  ...
  return <div className="space-y-6 animate-fade-in">
    ...
    <section className="rounded-3xl border border-violet-400/20 bg-[linear-gradient(135deg,rgba(124,58,237,.12),rgba(0,0,0,.18))] p-5 sm:p-6">
      ...
      <div className="mt-6 grid gap-5 lg:grid-cols-[.8fr_1.2fr]">
        ...
        <div className="rounded-2xl border border-white/7 bg-black/20 p-4"><div className="flex items-center justify-between"><p className="text-[9px] font-black uppercase tracking-widest text-slate-500">Historial manual</p><Lightbulb className="h-4 w-4 text-amber-300"/></div><div className="mt-3 max-h-64 space-y-2 overflow-y-auto">{!selected?.legacyBonuses?.length ? <p className="rounded-xl border border-dashed border-white/10 p-8 text-center text-xs text-slate-500">Sin bonificaciones manuales registradas.</p> : [...selected.legacyBonuses].reverse().map(item => <div key={item.id} className="flex items-center gap-3 rounded-xl border border-white/5 bg-white/[0.025] p-3"><div className="flex-1"><p className="text-xs font-bold text-white">{bonusConfig[item.type].label}{item.quantity > 1 ? ` × ${item.quantity}` : ''}</p><p className="mt-0.5 text-[9px] text-slate-500">{item.date}{item.note ? ` · ${item.note}` : ''}</p></div><strong className="text-xs text-violet-300">+{item.points} PA</strong><button onClick={() => deleteLegacyBonus(item.id)} aria-label="Eliminar bonificación" className="rounded-lg p-2 text-slate-600 hover:bg-rose-500/10 hover:text-rose-300"><Trash2 className="h-3.5 w-3.5"/></button></div>)}</div></div>
      </div>
    </section>
  </div>;
}
