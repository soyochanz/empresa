...
export default function LandingScreen({ onNavigate, projects }: LandingScreenProps) {
 ...
 <div className="relative min-h-screen w-full bg-[#020204] text-slate-100 flex flex-col overflow-hidden font-sans select-none">
   ...
   <div className="max-w-2xl flex-1 space-y-5">
    ...
    >
     <svg
      viewBox="0 0 24 24"
      aria-hidden="true"
      className="h-8 w-8 shrink-0 fill-current text-white"
     >
      <path d="M16.365 1.43c0 1.14-.417 2.096-1.252 2.867-.898.835-1.87 1.315-2.913 1.238-.125-1.097.32-2.273 1.055-3.03C14.085 1.65 15.487 1 16.365 1.43Zm4.09 16.75c-.547 1.253-.81 1.812-1.516 2.92-.984 1.548-2.37 3.476-4.088 3.49-1.527.014-1.92-1.01-3.992-.996-2.07.01-2.5 1.014-4.028 1-1.72-.016-3.033-1.754-4.02-3.3C.065 16.99-.228 11.94 1.62 9.25c1.31-1.91 3.38-3.03 5.33-3.03 1.986 0 3.235 1.086 4.877 1.086 1.594 0 2.565-1.09 4.864-1.09 1.738 0 3.577.948 4.883 2.58-4.29 2.35-3.595 8.48.88 9.384Z" />
     </svg>
  ...
  <motion.section
   ...
   viewport={{ once: true, margin: "-100px" }}
   transition={{ duration: 0.7 }}
   className="relative overflow-hidden border-y border-white/5 bg-[#07070b] py-16 sm:py-20"
  >
   <div className="absolute inset-0 bg-[linear-gradient(120deg,rgba(168,85,247,0.10),transparent_34%,rgba(14,165,233,0.08)_68%,transparent)] pointer-events-none" />
   <div className="relative max-w-7xl mx-auto px-6 grid lg:grid-cols-[0.9fr_1.1fr] gap-10 items-center">
    <div className="space-y-5">
     <div className="inline-flex items-center gap-2 rounded-full border border-cyan-400/20 bg-cyan-400/5 px-3 py-1.5 text-[10px] font-mono font-bold uppercase tracking-[0.22em] text-cyan-300">
      <Megaphone className="w-3.5 h-3.5" />
      Redes sociales gestionadas
     </div>
     <div className="space-y-3">
      <h2 className="text-3xl sm:text-5xl font-black tracking-tight text-white leading-tight">
       Contenido, comunidad y crecimiento con criterio de marca.
      </h2>
      <p className="text-sm sm:text-base text-slate-400 leading-relaxed max-w-xl">
       Diseñamos calendarios editoriales, piezas visuales, copies, campañas y reportes para que tu presencia en redes se vea consistente, activa y orientada a conversión.
      </p>
     </div>
    </div>
    ...
    <div className="grid sm:grid-cols-3 gap-3">
     {[
      { icon: Instagram, title: 'Contenido visual', text: 'Reels, carruseles y creatividades adaptadas a cada canal.', color: 'text-pink-300 border-pink-400/20 bg-pink-400/5' },
      { icon: Share2, title: 'Plan editorial', text: 'Publicaciones ordenadas por objetivo, audiencia y temporada.', color: 'text-violet-300 border-violet-400/20 bg-violet-400/5' },
      { icon: BarChart3, title: 'Métricas claras', text: 'Lectura mensual de alcance, interacción y oportunidades reales.', color: 'text-cyan-300 border-cyan-400/20 bg-cyan-400/5' }
     ].map(item => (
      <motion.div
       key={item.title}
       whileHover={{ y: -4 }}
       className={`rounded-2xl border ${item.color} p-5 min-h-[190px] flex flex-col justify-between`}
      >
  ...
  >
   ...
   >
    ...
    <h3 className="font-bold text-sm text-white">¡Solicitud Enviada con ÉÉxito!</h3>
  ...
  <AnimatePresence>
   ...
   >
    <motion.div
     ...
     className="w-full max-w-2xl rounded-3xl border border-white/10 bg-[#08080d] p-6 sm:p-8 shadow-2xl"
    >
     <div className="flex items-start justify-between gap-4 mb-5">
      <div>
       <p className="text-[10px] font-mono uppercase tracking-[0.25em] text-violet-300">
        Althera Solutions
       </p>
       <h3 className="text-2xl font-black text-white mt-1">
        {legalModal === 'terms' ? 'Términos de uso' : 'Política de privacidad'}
       </h3>
      </div>
      <button
       type="button"
       onClick={() => setLegalModal(null)}
       className="p-2 rounded-xl border border-white/10 text-slate-400 hover:text-white hover:bg-white/5 transition cursor-pointer"
     ...
     {legalModal === 'terms' ? (
      <div className="space-y-3 text-sm leading-relaxed text-slate-400">
       <p>El uso de esta web implica aceptar un acceso responsable a la información comercial publicada por Althera Solutions.</p>
       <p>Las propuestas, diseños, demostraciones y contenidos mostrados tienen finalidad informativa y podrán adaptarse según el alcance acordado con cada cliente.</p>
       <p>Queda prohibido copiar, revender o explotar materiales de Althera sin autorización expresa por escrito.</p>
      </div>
     ) : (
      <div className="space-y-3 text-sm leading-relaxed text-slate-400">
       <p>Los datos enviados mediante formularios se utilizan únicamente para responder solicitudes comerciales, preparar propuestas y mantener comunicaciones relacionadas.</p>
       <p>No vendemos datos personales a terceros. El acceso interno está limitado al equipo autorizado y se gestiona con controles de autenticación.</p>
       <p>Puedes solicitar revisión, actualización o eliminación de tus datos escribiendo al canal de contacto de Althera.</p>
      </div>
     )}
    </motion.div>
   </motion.div>
