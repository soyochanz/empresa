...
export default function LegacyRankSystem({ comercial, comercialesList, finTransactions, events, coldLeads, contacts }: Props) {
  ...
  return <div className="space-y-6">
    ...
    <section><div className="mb-4"><p className="text-[10px] font-black uppercase tracking-[.22em] text-slate-500">Camino de rangos</p><h3 className="mt-1 text-lg font-bold text-white">Tu legado visual</h3></div><div className="grid grid-cols-3 gap-3 lg:grid-cols-6">{LEGACY_RANKS.map(rank => { const active = rank.name === legacy.rank.name; const unlocked = legacy.total >= rank.min; return <div key={rank.name} className={`relative rounded-2xl border p-3 text-center transition ${active ? 'border-violet-400/40 bg-violet-500/10 ring-1 ring-violet-400/20' : unlocked ? 'border-white/10 bg-white/[0.035]' : 'border-white/5 bg-black/20 opacity-40 grayscale'}`}><img src={rank.asset} alt={`Rango ${rank.name}`} className={`legacy-rank-badge mx-auto h-24 w-24 object-contain ${rank.name === 'Diamante' ? 'legacy-rank-badge--diamond' : ''}`}/><p className="mt-1 text-xs font-black uppercase text-white">{rank.name}</p><p className="mt-1 text-[9px] text-slate-500">{rank.min.toLocaleString('es-ES')} PA{rank.max !== Infinity ? ` · ${rank.max.toLocaleString('es-ES')}` : '+'}</p>{active && <span className="absolute right-2 top-2 h-2 w-2 animate-pulse rounded-full bg-violet-300"/>}</div>})}</div></section>
