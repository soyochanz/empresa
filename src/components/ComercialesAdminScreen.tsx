...
export default function ComercialesAdminScreen({
 ...
 try {
  ...
  'Completa la configuración de Stripe Connect del comercial. Cuando Stripe active la capacidad de transfers, podrás liquidar comisiones reales.'
  );
 } catch (err: any) {
 ...
 ...coldLeads
  .filter(lead => lead.assignedToEmail && lead.assignedToEmail !== 'unassigned')
  .slice(0, 8)
  .map(lead => ({
  id: `cold_${lead.id}`,
  text: `${lead.assignedToName || lead.assignedToEmail} tiene asignado ${lead.businessName}`,
  meta: `${lead.callsCount || 0} llamadas ${lead.callDate || lead.createdAt?.slice(0, 10) || 'reciente'}`
  })),
 ...mappedLeadsList.slice(0, 8).map(lead => ({
  id: `lead_${lead.id}`,
  text: `${lead.comercialName} movio ${lead.company || lead.name} a ${lead.status}`,
  meta: lead.createdAt ? new Date(lead.createdAt).toLocaleDateString('es-ES') : 'reciente'
 })),
 ...finTransactions
  .filter(tx => tx.comercialId || tx.comercialEmail)
  .slice(0, 8)
  .map(tx => ({
  id: `tx_${tx.id}`,
  text: `${tx.comercialEmail || 'Comercial'} ${tx.status === 'paid' ? 'cobró' : 'pendiente'} ${Number(tx.amount || 0).toLocaleString('es-ES')} EUR`,
  meta: tx.date || 'reciente'
  }))
 ].slice(0, 10);
 ...
 <div className="p-8 space-y-8 flex-1 overflow-y-auto bg-transparent text-slate-100 relative min-h-screen">
  ...
  <div className="space-y-6 animate-fade-in">
   ...
   <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
    ...
    {/* DATOS BANCARIOS VISIBLES PARA ADMIN */}
    <div className="bg-[#020205]/40 border border-emerald-500/15 rounded-2xl p-5 space-y-4">
     <div className="flex items-center justify-between">
     <div>
      <h4 className="text-xs font-bold text-white uppercase tracking-wider font-mono">Datos bancarios del comercial</h4>
      <p className="text-[10px] text-slate-500 mt-1">Información guardada por el comercial para liquidaciones manuales o transferencia.</p>
     </div>
     <span className={`text-[9px] font-mono px-2 py-0.5 rounded border font-bold ${
      ...
      {currentComercial.iban ? 'Completo' : 'Pendiente'}
     </span>
     </div>

     <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
     <div className="rounded-xl bg-slate-950/50 border border-white/5 p-3">
      <span className="text-[8px] font-mono text-slate-500 uppercase tracking-widest block">Banco</span>
      <p className="text-xs text-white font-bold mt-1 break-words">{currentComercial.bankName || 'Sin banco guardado'}</p>
     </div>
     <div className="rounded-xl bg-slate-950/50 border border-white/5 p-3">
      <span className="text-[8px] font-mono text-slate-500 uppercase tracking-widest block">IBAN</span>
      <p className="text-xs text-white font-bold mt-1 break-all font-mono">{currentComercial.iban || 'Sin IBAN guardado'}</p>
     </div>
     <div className="rounded-xl bg-slate-950/50 border border-white/5 p-3">
      <span className="text-[8px] font-mono text-slate-500 uppercase tracking-widest block">BIC / SWIFT</span>
      <p className="text-xs text-white font-bold mt-1 break-words font-mono">{currentComercial.bic || 'Sin BIC guardado'}</p>
     </div>
     </div>

     {currentComercial.stripeConnectAccountId && (
     <p className="text-[10px] font-mono text-emerald-400 bg-emerald-500/5 border border-emerald-500/10 rounded-xl px-3 py-2">
     ...
     return (
      ...
      <div className="bg-slate-950/50 p-3.5 rounded-xl border border-white/5 space-y-2 text-left">
       ...
       <div className="text-[10px] text-amber-400 bg-amber-500/5 p-2 rounded-lg border border-amber-500/10 leading-normal font-sans">
        ⚠ El comercial no ha configurado sus datos bancarios todavía. Solicítale que los rellene para poder liquidar.
       </div>
       )}
      </div>
      ...
      <button
       ...
       const methodLabel = payoutMethod === 'stripe' ? 'Stripe Direct' : payoutMethod === 'cash' ? 'cash' : 'transferencia bancaria';
       triggerConfirm(
        'Confirmar Liquidación',
        `¿Confirmas que deseas liquidar ${indPendingCommission.toLocaleString('es-ES', { style: 'currency', currency: 'EUR' })} a ${currentComercial.name} mediante ${methodLabel}?`,
        () => {
    ...
    <div className="bg-[#020205]/40 border border-white/5 rounded-2xl p-5 space-y-4">
     ...
     <p className="text-[10px] text-slate-500 italic font-sans py-2">
      No hay registros de liquidaciones previas para este comercial.
     </p>
     ) : (
     <div className="space-y-2.5 max-h-60 overflow-y-auto pr-1">
      ...
      const methodLabel = method === 'stripe' ? `Stripe ${p.stripeTransferId || ''}` : method === 'cash' ? 'Cash' : 'Transferencia';
      return (
      <div key={p.id} className="bg-[#040409] p-3 rounded-xl border border-white/5 flex flex-col gap-1.5 text-left">
       ...
       <div className="text-slate-500 font-sans">
        <span className="text-slate-400 font-semibold">{method === 'cash' ? 'Cash' : p.bankName}</span> • {p.bankAccount}
       </div>
  ...
  <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start animate-fade-in">
   ...
   <div className="bg-amber-500/[0.03] border border-amber-500/15 rounded-2xl p-5 backdrop-blur-xl space-y-4">
    <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
    <div>
     <h3 className="font-bold text-sm text-white text-left flex items-center gap-2">
     <Coins className="w-4 h-4 text-amber-400" />
     <span>Comisiónes extra manuales</span>
     </h3>
     <p className="text-[10px] text-slate-400 mt-1">Se suman a las comisiones automáticas sin alterar tiers ni reglas existentes.</p>
    </div>
    <div className="flex rounded-xl border border-white/10 bg-black/20 p-1">
     {(['manual', 'income'] as const).map(mode => (
     <button key={mode} type="button" onClick={() => setExtraCommissionMode(mode)} className={`px-3 py-1.5 rounded-lg text-[10px] font-black uppercase ${extraCommissionMode === mode ? 'bg-amber-500 text-black' : 'text-slate-400 hover:text-white'}`}>
      {mode === 'manual' ? 'Euros' : '% ingreso'}
     </button>
     ))}
    </div>
    </div>
    ...
    {extraCommissionMode === 'manual' ? (
     <input type="number" min="0" step="0.01" value={extraCommissionAmount} onChange={e => setExtraCommissionAmount(e.target.value)} placeholder="Importe EUR" className="bg-black/40 border border-white/10 rounded-xl px-3 py-2 text-xs text-white outline-none focus:border-amber-500" />
    ) : (
     <>
   ...
   <div className="bg-white/[0.02] border border-white/5 rounded-2xl p-5 backdrop-blur-xl">
    <h3 className="font-bold text-sm text-white mb-4 text-left flex items-center gap-2">
    <Activity className="w-4 h-4 text-emerald-400" />
    <span>Activity Feed Comerciales</span>
    </h3>
    <div className="space-y-3 max-h-56 overflow-y-auto pr-1">
    {commercialActivity.length === 0 ? (
     <p className="text-xs text-slate-500">Todavia no hay actividad comercial registrada.</p>
    ) : commercialActivity.map(item => (
     <div key={item.id} className="flex gap-3 rounded-xl border border-white/5 bg-black/20 p-3">
     ...
     <div className="min-w-0">
      ...
      <p className="text-[9px] font-mono text-slate-500 mt-0.5">{item.meta}</p>
     </div>
     </div>
    ))}
    </div>
      ...
      return (
       ...
       <td className="py-4 pl-2">
        ...
        <div>
         ...
         Credencial protegida: <strong className="text-slate-400">••••••••</strong>
         </p>
        </div>
        </div>
       </td>
       ...
       <td className="py-4 text-right">
        ...
        {extraForC > 0 && (
         <span className="block text-[8px] font-mono text-amber-400">+ {extraForC.toLocaleString('es-ES')} EUR extra</span>
        )}
