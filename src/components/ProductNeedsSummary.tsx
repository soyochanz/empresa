...
export default function ProductNeedsSummary({ products = [], otherDetail, compact = false, className = '' }: ProductNeedsSummaryProps) {
 ...
 if (compact) {
  return (
   <section className={`rounded-xl border border-cyan-300/15 bg-gradient-to-br from-cyan-400/[0.06] via-violet-400/[0.03] to-transparent p-3 ${className}`}>
    <div className="flex items-center gap-2">
     <Sparkles className="h-3.5 w-3.5 shrink-0 text-cyan-200" />
     <p className="text-[8px] font-black uppercase tracking-[.14em] text-cyan-200">Qué necesita este lead</p>
    </div>
    <div className="mt-2 flex flex-wrap gap-1.5">
     {uniqueProducts.map(product => {
      const visual = resolveProductVisual(product);
      return (
       <span key={product} className={`inline-flex max-w-full items-center gap-1.5 rounded-lg border px-2 py-1.5 ${visual.style}`}>
        <visual.Icon className="h-3 w-3 shrink-0" />
        <strong className="break-words text-[8px] leading-3">{visual.label || product}</strong>
       </span>
      );
     })}
    </div>
    {otherDetail?.trim() && (
     <p className="mt-2 break-words border-t border-white/[0.06] pt-2 text-[8px] leading-3.5 text-slate-400">
      <strong className="text-emerald-200">Otro: </strong>{otherDetail.trim()}
     </p>
    )}
   </section>
  );
 ...
 return (
  <section className={`rounded-2xl border border-cyan-300/15 bg-gradient-to-br from-cyan-400/[0.07] via-violet-400/[0.04] to-transparent p-4 ${className}`}>
