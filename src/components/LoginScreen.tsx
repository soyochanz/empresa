...
export default function LoginScreen({ onSignIn, onBackToLanding }: LoginScreenProps) {
 ...
 try {
  ...
  if (data.user && !data.session) {
   setSuccessMsg('¡Registro eÉxitoso! Por favor verifica tu casilla de correo electrónico para confirmar tu cuenta y poder iniciar sesión.');
   // Reset form to Login view
   setIsSignUp(false);
  } else if (data.session) {
   setSuccessMsg('¡Registro eÉxitoso! Iniciando sesión...');
