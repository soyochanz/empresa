...
export default function DeveloperHubScreen({
  ...
  const handlePromoteToPortfolio = (contact: ClientContact) => {
    ...
    const newProj: AgencyProject = {
      ...
      url: contact.website || `${contact.company.toLowerCase().replace(/[^a-z0-9]/g, '')}.althera.dev`,
        ...
        <div className="xl:col-span-5">
          {selectedContact ? (
            <div className="bg-white/[0.015] border border-white/5 rounded-3xl p-6 space-y-6 relative text-left">
              ...
              {/* Publish to portfolio action card button */}
              <div className="pt-4 border-t border-white/5">
                <button
                  type="button"
                  onClick={() => handlePromoteToPortfolio(selectedContact)}
                  className="w-full py-2.5 bg-gradient-to-r from-violet-600 to-indigo-600 hover:from-violet-500 hover:to-indigo-500 text-white rounded-2xl text-xs font-semibold flex items-center justify-center gap-2 transition cursor-pointer shadow-lg shadow-violet-500/10"
                >
                  <Globe className="w-4 h-4" />
                  Publicar en Portafolio Público (Lighthouse ready)
                </button>
                <p className="text-[9px] text-slate-500 text-center mt-1.5 leading-normal">
                  Esto generará un registro dinámico en el panel de Proyectos y en el showcase de la landing page para Althera.
                </p>
              </div>
